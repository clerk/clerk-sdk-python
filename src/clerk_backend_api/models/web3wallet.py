...
class VerificationAdminVerificationAdmin(BaseModel):
    ...
    def serialize_strategy(self, value):
        if isinstance(value, str):
            try:
                return models.VerificationAdminVerificationWeb3WalletStrategy(value)
            except ValueError:
                return value
        return value
...
class Web3Signature(BaseModel):
    ...
    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["object", "nonce", "message", "verified_at_client"])
        nullable_fields = set(
            ["nonce", "message", "attempts", "expire_at", "verified_at_client"]
        )
        serialized = handler(self)
        m = {}
...
class Web3Wallet(BaseModel):
    ...
    def serialize_model(self, handler):
        ...
        nullable_fields = set(["verification"])
        serialized = handler(self)
        m = {}
